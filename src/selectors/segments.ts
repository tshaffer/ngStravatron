...
export const getSegment = (state: StravaModelState, segmentId: number): StravatronDetailedSegment => {
  const segments: SegmentsMap = getSegments(state);
  const segment: StravatronDetailedSegment = segments[segmentId];
  return segment;
};
